...
const extracted = `SANDRINE|||https://www.tangierlatinfestival.com/wp-content/uploads/2024/04/image-228-2.png
...
const isSchool = (name) => {
  ...
  return upper.includes("DANCE") || upper.includes("TEAM") || upper.includes("ACADEMY") || upper.includes("SCHOOL") || upper.includes("LADIES") || upper.includes("COMPANY") || upper.includes("B.MAMBO") || upper.includes("EVOLADIES");
};
...
extracted.trim().split('\\n').forEach(line => {
  ...
  if (mappings[upperName]) {
    ...
    style = "Ecole"; // Let's use Ecole for dance school
  }
  ...
  let cleanName = name;
  // If it's all uppercase, Title Case it for UI beauty
  if (cleanName === cleanName.toUpperCase() && !isDJ(name)) {
     cleanName = ucFirst(cleanName);
  }

  output += '  { name: "' + cleanName.replace(/"/g, '\\"') + '", style: "' + style + '", img: ' + finalImg + ' },\n';
});
output += '];\n';
