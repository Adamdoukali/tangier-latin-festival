    ...
    <div className="hidden md:block relative z-[60] border-b border-border/40 bg-background/60 backdrop-blur">
      <div className="mx-auto max-w-7xl px-6 py-2 flex items-center justify-between text-xs text-muted-foreground">
        ...
        <div className="flex items-center gap-6">
          ...
          <a
            href="tel:+212664010279"
            className="flex items-center gap-2 hover:text-primary transition"
          >
            <Phone className="h-3.5 w-3.5" />
            <span>+212 6 64 01 02 79</span>
          </a>
    ...
    <header className="sticky top-0 z-50">
      ...
      <div className="relative z-[55] border-b border-border/40 bg-background/70 backdrop-blur-xl shadow-[0_4px_12px_-4px_rgba(0,0,0,0.08)]">
        <div className="mx-auto max-w-7xl px-4 xl:px-6 h-20 flex items-center justify-between gap-4">
          ...
          {/* Desktop Nav */}
          <nav className="hidden 2xl:flex items-center justify-center gap-1 2xl:gap-2 flex-1 min-w-0">
            {links.map((l) => (
              <a
                key={l.href}
                href={localizedHref(l.href)}
                className="text-foreground/80 hover:text-primary transition tracking-wider uppercase text-[10px] 2xl:text-[11px] font-semibold whitespace-nowrap px-1 py-2"
              >
                {l.label}
              </a>
            ))}
          ...
          <div className="hidden 2xl:flex items-center gap-3 shrink-0">
            ...
            <div className="flex items-center gap-3 border border-gold/25 bg-background/45 backdrop-blur-xl rounded-full p-1.5 shadow-gold hover:border-gold/55 transition duration-300">
              {languages.map((l) => (
                <button
                  key={l.code}
                  onClick={() => changeLanguage(l.code)}
                  className={`group relative h-9 w-9 rounded-full flex items-center justify-center text-lg transition-all duration-500 cursor-pointer overflow-visible ${
                    lang === l.code
            ...
            <a
              ...
              className="inline-flex items-center gap-2 rounded-full bg-gold px-5 py-2.5 text-sm font-medium text-primary-foreground shadow-gold hover:opacity-90 transition"
            >
              {t("buyPackBtn")}
            </a>
          </div>
          ...
          <div className="flex items-center 2xl:hidden gap-3">
            <button
            ...
            <nav className="flex flex-col gap-4 overflow-y-auto max-h-[55vh] pr-2">
              {links.map((l) => (
                <a
                  ...
                  href={localizedHref(l.href)}
                  onClick={() => setMobileMenuOpen(false)}
                  className="text-foreground/90 hover:text-primary transition font-display text-base tracking-wider uppercase border-b border-border/10 pb-2"
                >
                  {l.label}
                </a>
              ))}
